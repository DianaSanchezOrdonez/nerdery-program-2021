...
function Slider(slider) {
  ...
  var prev;
  var current;
  var next; //select the element needed for the slider
  ...
  function startSlider() {
    ...
    next = current.nextElementSibling || slides.firstElementChild; // console.log({current, prev, next});
  }
  ...
  function move(direction) {
    ...
    var classToRemove = ['prev', 'current', 'next']; // prev.classList.remove('prev', 'current', 'next');

    (_prev$classList = prev.classList).remove.apply(_prev$classList, classToRemove);

    (_current$classList = current.classList).remove.apply(_current$classList, classToRemove);

    (_next$classList = next.classList).remove.apply(_next$classList, classToRemove);

    if (direction === 'back') {
      //make an new array of the new values, and destructure
      //them over and into the prev, current and next variables
      var _ref = [//get the prev slide, if there is none, get the last slide
      ...
      var _ref2 = [current, next, //get the next slide, or if it's at the end, loop around
  ...
  startSlider();
  applyClasses(); //buttons event listeners
